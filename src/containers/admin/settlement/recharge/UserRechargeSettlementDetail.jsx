...
let content = {float:"left",width:"22%",marginLeft:"1%"}
...
let input = {marginLeft:"29px",textIndent:"5px",width:"230px",height:"35px",border:"1px solid #ccc"}
...
export default class UserRechargeSettlementDetail extends Component {
    ...
    detailListdata(){
       let datailId = sessionStorage.getItem("UserDetailkey")
        ...
        $.ajax(settings).done(
            function (response) {
                if (response.result === 1) {
                    ...
                    sessionStorage.setItem("UserRechargeDetaillistData",str)
    ...
    endTime(e, value) {
     ...
     //设置分页数据内容
    setPage(num){
        let  b = JSON.parse(sessionStorage.getItem("UserRechargeDetaillistData")) 
        this.setState({
   ...
   //分页事件
     pageNext (num) {
        let  c =  JSON.parse(sessionStorage.getItem("UserRechargeDetaillistData"))
        if(c){
