...
export default class UserRCGotStatistics extends Component {
    constructor(props) {
        super(props);
    ...
    listData() {
        ...
        $.ajax(settings).done(function (response) {
            if (response.result === 1) {
                ...
                sessionStorage.setItem("UserGolistData",str)
    ...
    SearchClick() {
        ...
        let daiLi = this.refs.daiLi.value//代理
        let userId = this.refs.userId.value//用户ID
        let userName = this.refs.userName.value//用户昵称
        let newmyDate = new Date();//end时间
        let end = newmyDate.getTime()//毫秒
    ...
    //设置分页数据内容
    setPage(num){
        let  b = JSON.parse(sessionStorage.getItem("UserGolistData")) 
        this.setState({
   ...
   //分页事件
     pageNext (num) {
        let c =  JSON.parse(sessionStorage.getItem("UserGolistData"))
        if(c){
            ...
            <div style={{minWidth: "1000px"}}>
               <div style={{height:"72px"}}>
                <div style={contentTime}>
                          <label style={{fontSize:"16px",lineHeight:"72px",float:"left"}}>充值时间区间</label>
                          <DatePicker
                            ref="time"
                ...
                <div style={content}>
                  <label style={label}>代理</label>
                  <input type="text" ref="daiLi" style={input} placeholder="请输入代理"/>
                </div>
                <div style={content}>
                ...
                <div style={{textAlign: "left"}}>
                    ...
                    <button style={style} onClick={this.ExcelOut.bind(this)}>导出</button>
                ...
                <Table>
                    <TableHeader adjustForCheckbox={false} displaySelectAll={false}>
                        <TableRow>
                            ...
                            <TableHeaderColumn>发放房卡数量</TableHeaderColumn>
                            <TableHeaderColumn>充值代理</TableHeaderColumn>
                            <TableHeaderColumn>代理等级</TableHeaderColumn>
                        </TableRow>
                    </TableHeader>
